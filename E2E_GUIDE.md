...
- Test code may not use arbitrary sleeps, delayed dispatch, or test retries. After semantic state passes, the step helper uses an XCTest predicate to require two consecutive pixel-identical frames within two seconds; this is a rendering-stability gate, not a tolerance or retry.
